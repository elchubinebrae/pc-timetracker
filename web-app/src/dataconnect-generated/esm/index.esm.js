...
export const listTasksForUserRef = (dc) => {
  const { dc: dcInstance} = validateArgs(connectorConfig, dc, undefined);
  dcInstance._useGeneratedSdk();
...
export function listTasksForUser(dc) {
  return executeQuery(listTasksForUserRef(dc));
}

export const logTimeRef = (dc) => {
  const { dc: dcInstance} = validateArgs(connectorConfig, dc, undefined);
  dcInstance._useGeneratedSdk();
  return mutationRef(dcInstance, 'LogTime');
}
logTimeRef.operationName = 'LogTime';

export function logTime(dc) {
  return executeMutation(logTimeRef(dc));
}
